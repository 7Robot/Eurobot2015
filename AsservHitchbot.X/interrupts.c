...
void __attribute__ ((__interrupt__, no_auto_psv)) _CNInterrupt(void)
{
    ...
    // baisse le flag puis rÈcup des etats de pins, 
    // si les pins rebougent durant ce laps tres court, Áa redÈclenchera une IT directe apres,
    // mais avec old_ Áa doit tenir
    ...
    // si Etat_Ultrason mÈrite que l'on s'occupe de lui
    if (Etat_Ultrason & (U_ETAT_WAIT1 + U_ETAT_WAIT0 + U_ETAT_WAIT0_OVERSHOOT)) {
        if (Etat_Pin_Ultrason) {
            ...
            if (Etat_Ultrason & U_ETAT_WAIT0) {     // si attente standard => rÈcup mesure
                ...
                if (Sector_Ultrason) {
                    if (Mesure_Distance_Ultrason < (ULTRASON_THRESOLD - ULTRASON_THRESOLD_TRIGGER)) {
                        ...
                        Sector_Ultrason = 0;            // passage en sector  occupÈ
